...
type GrafanaLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Grafana, err error)
...
// List lists all Grafanas in the indexer.
func (s *grafanaLister) List(selector labels.Selector) (ret []*v1beta1.Grafana, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Grafana))
	})
	return ret, err
}
...
type GrafanaNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Grafana, error)
...
// List lists all Grafanas in the indexer for a given namespace.
func (s grafanaNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Grafana, err error) {
...
// Get retrieves the Grafana from the indexer for a given namespace and name.
func (s grafanaNamespaceLister) Get(name string) (*v1beta1.Grafana, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("grafana"), name)
	}
	return obj.(*v1beta1.Grafana), nil
}
