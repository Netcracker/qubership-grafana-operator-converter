...
type GrafanaFolderLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.GrafanaFolder, err error)
...
// List lists all GrafanaFolders in the indexer.
func (s *grafanaFolderLister) List(selector labels.Selector) (ret []*v1beta1.GrafanaFolder, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.GrafanaFolder))
	})
	return ret, err
}
...
type GrafanaFolderNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.GrafanaFolder, error)
...
// List lists all GrafanaFolders in the indexer for a given namespace.
func (s grafanaFolderNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.GrafanaFolder, err error) {
...
// Get retrieves the GrafanaFolder from the indexer for a given namespace and name.
func (s grafanaFolderNamespaceLister) Get(name string) (*v1beta1.GrafanaFolder, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("grafanafolder"), name)
	}
	return obj.(*v1beta1.GrafanaFolder), nil
}
