...
type GrafanaDashboardLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.GrafanaDashboard, err error)
...
// List lists all GrafanaDashboards in the indexer.
func (s *grafanaDashboardLister) List(selector labels.Selector) (ret []*v1beta1.GrafanaDashboard, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.GrafanaDashboard))
	})
	return ret, err
}
...
type GrafanaDashboardNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.GrafanaDashboard, error)
...
// List lists all GrafanaDashboards in the indexer for a given namespace.
func (s grafanaDashboardNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.GrafanaDashboard, err error) {
...
// Get retrieves the GrafanaDashboard from the indexer for a given namespace and name.
func (s grafanaDashboardNamespaceLister) Get(name string) (*v1beta1.GrafanaDashboard, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("grafanadashboard"), name)
	}
	return obj.(*v1beta1.GrafanaDashboard), nil
}
