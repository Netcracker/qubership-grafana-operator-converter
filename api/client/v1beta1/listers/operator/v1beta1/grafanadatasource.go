...
type GrafanaDatasourceLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.GrafanaDatasource, err error)
...
// List lists all GrafanaDatasources in the indexer.
func (s *grafanaDatasourceLister) List(selector labels.Selector) (ret []*v1beta1.GrafanaDatasource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.GrafanaDatasource))
	})
	return ret, err
}
...
type GrafanaDatasourceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.GrafanaDatasource, error)
...
// List lists all GrafanaDatasources in the indexer for a given namespace.
func (s grafanaDatasourceNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.GrafanaDatasource, err error) {
...
// Get retrieves the GrafanaDatasource from the indexer for a given namespace and name.
func (s grafanaDatasourceNamespaceLister) Get(name string) (*v1beta1.GrafanaDatasource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("grafanadatasource"), name)
	}
	return obj.(*v1beta1.GrafanaDatasource), nil
}
