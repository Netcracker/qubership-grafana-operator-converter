...
const GrafanaFolderKind = "GrafanaFolder"

// GrafanaPermissionItem defines the _complete_ permissions for the folder.
...
// GrafanaFolderSpec defines the desired state of GrafanaDashboard
// +k8s:openapi-gen=true
...
func (f *GrafanaFolder) Hash() string {
	...
	io.WriteString(hash, f.ObjectMeta.Namespace) // nolint
...
func (f *GrafanaFolder) GetPermissions() []*GrafanaPermissionItem {
	permissions := make([]*GrafanaPermissionItem, len(f.Spec.FolderPermissions))
	for i, p := range f.Spec.FolderPermissions {
		permissions[i] = &p
	}
